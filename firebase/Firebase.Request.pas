...
function TFirebaseRequest.SendData(const AResourceParams: array of string; const ACommand: TFirebaseCommand; AData: TJSONValue = nil;
  AQueryParams: TDictionary<string, string> = nil; ADataOwner: boolean = true): IFirebaseResponse;
var
  LClient: THTTPClient;
  LResp: IHTTPResponse;
...
begin
  try
    ...
    try
      ...
      try
        LURL := BaseURI + EncodeResourceParams(AResourceParams) + EncodeToken(Token) + EncodeQueryParams(AQueryParams);
        case ACommand of
          fcPut:
...
function TFirebaseRequest.EncodeQueryParams(AQueryParams: TDictionary<string, string>): string;
var
...
begin
  ...
  begin
    if Result <> '?' then
      Result := Result + '&';
    Result := Result + TNetEncoding.URL.URLDecode(Param.Key) + '=' + TNetEncoding.URL.URLDecode(Param.Value)
  end;
end;

function TFirebaseRequest.EncodeResourceParams(AResourceParams: array of string): string;
var
  i: integer;
begin
  Result := '';
